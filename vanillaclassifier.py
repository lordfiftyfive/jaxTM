...
class TMClassifier(TMBaseModel, MultiClauseBankMixin, MultiWeightBankMixin):
    ...
    def init_after(self, X: np.ndarray, Y: np.ndarray):
        self.positive_clauses = jnp.concatenate((jnp.ones(self.number_of_clauses // 2, dtype=jnp.int32),
                                                jnp.zeros(self.number_of_clauses // 2, dtype=jnp.int32)))

        self.negative_clauses = jnp.concatenate((jnp.zeros(self.number_of_clauses // 2, dtype=jnp.int32),
                                                jnp.ones(self.number_of_clauses // 2, dtype=jnp.int32)))
    ...
    def mechanism_clause_active(self) -> np.ndarray:
        ...
        return (self.rng.rand(len(self.weight_banks), self.number_of_clauses) >= self.clause_drop_p).astype(jnp.int32)
    ...
    def predict_compute_class_sums(
        ...
        for ith_class in self.weight_banks.classes():
            ...
            if clip_class_sum:
                class_sum = jnp.clip(class_sum, -self.T, self.T)
            class_sums.append(class_sum)
        return class_sums

    def transform(self, X):
        encoded_X = self.clause_banks[0].prepare_X(X)
        transformed_X = jnp.empty((X.shape[0], len(self.weight_banks), self.number_of_clauses), dtype=np.uint32)
    ...
    def literal_clause_frequency(self):
        clause_active = jnp.ones(self.number_of_clauses, dtype=np.uint32)
        literal_frequency = jnp.zeros(self.clause_banks[0].number_of_literals, dtype=np.uint32)
        for i in self.weight_banks.classes():
